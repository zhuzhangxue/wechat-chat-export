...
class App(tk.Tk):
    def __init__(self):
        ...
        ttk.Entry(outrow, textvariable=self.out_var).pack(side="left", fill="x", expand=True, padx=(8, 8))
        ...
        self.open_btn = ttk.Button(bottom, text="打开导出文件夹", command=self.open_output, state="disabled")
    ...
    def choose_out(self):
        path = filedialog.askdirectory(initialdir=self.out_var.get() or str(app_dir()))
        if path:
    ...
    def start_export(self):
        ...
        thread = threading.Thread(
            target=self.worker,
            args=(name, self.out_var.get().strip() or str(app_dir() / "exports")),
            daemon=True,
        )
        thread.start()
    ...
    def worker(self, name, outdir):
        try:
            result = export_chat(name, outdir, progress=lambda m: self.q.put(("log", m)))
