...
# 与 EXE 文件图标使用同一套图形。
# 这里内嵌一份小尺寸 PNG，专门给 Tk 窗口标题栏使用，
# 避免 Windows 仍显示 Tk 默认的“羽毛”图标。
...
class App(tk.Tk):
    def __init__(self):
        ...
        # 改用 grid，让窗口变小时优先压缩日志区，
        # 不再把右下角“打开导出文件夹”按钮挤出可视范围。
        ...
        ttk.Separator(pad).grid(row=5, column=0, sticky="ew", pady=10)
