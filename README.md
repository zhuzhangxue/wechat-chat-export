...
- 勾选“视频”后，程序会使用这些标识尝试匹配本机仍存在的缓存，包括当前会话 `msg/attach/.../Video` 中的普通版和 `_raw` 视频；
- 无法可靠匹配时不会猜测或复制其他视频，JSON 仍保留解析结果并明确记录未找到本机缓存的原因；
- 当前不会因为视频号消息而自动联网下载 CDN 视频，避免把“本地导出”变成隐式网络请求。

视频号 `finder_feed` 中可能包含临时 CDN URL 等资源信息。导出的 JSON 本身就是聊天数据的一部分，不建议未经脱敏直接公开分享。
